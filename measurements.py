...
def get_measurements_from_data(shape, left_pupil, right_pupil, CalibrationType, CalibrationValue):
    ...
    cross_upperlid_left = rotate_axis(np.column_stack((x1_upperlid_left, y1_upperlid_left)), rot_angle,
                                      np.array([left_pupil[0], left_pupil[1]]))
    ...
    cross_lowerlid_left = rotate_axis(np.column_stack((x1_lowerlid_left, y1_lowerlid_left)), rot_angle,
                                      np.array([left_pupil[0], left_pupil[1]]))
    ...
    cross_brown_left = rotate_axis(np.column_stack((x1_brown_left, y1_brown_left)), rot_angle,
                                   np.array([left_pupil[0], left_pupil[1]]))
    ...
    cross_upperlid_right = rotate_axis(np.column_stack((x1_upperlid_right, y1_upperlid_right)), rot_angle,
                                       np.array([right_pupil[0], right_pupil[1]]))
    ...
    cross_lowerlid_right = rotate_axis(np.column_stack((x1_lowerlid_right, y1_lowerlid_right)), rot_angle,
                                       np.array([right_pupil[0], right_pupil[1]]))
    ...
    cross_brow_right = rotate_axis(np.column_stack((x1_brow_right, y1_brow_right)), rot_angle,
                                   np.array([right_pupil[0], right_pupil[1]]))
    ...
    ResultsDeviation.MarginalReflexDistance1 = abs(
        ResultsLeft.MarginalReflexDistance1 - ResultsRight.MarginalReflexDistance1)
    ResultsDeviation.MarginalReflexDistance2 = abs(
        ResultsLeft.MarginalReflexDistance2 - ResultsRight.MarginalReflexDistance2)
    ResultsDeviation.BrowHeight = abs(ResultsLeft.BrowHeight - ResultsRight.BrowHeight)
    ResultsDeviation.PalpebralFissureHeight = abs(
        ResultsLeft.PalpebralFissureHeight - ResultsRight.PalpebralFissureHeight)

    if shape[57, 0] >= cross_lowerlip[0]:  # left is the good side (probably)
        ResultsPercentile.BrowHeight = abs(
            ResultsLeft.BrowHeight - ResultsRight.BrowHeight) * 100 / ResultsLeft.BrowHeight
        ResultsPercentile.MarginalReflexDistance1 = abs(
            ResultsLeft.MarginalReflexDistance1 - ResultsRight.MarginalReflexDistance1) * 100 / ResultsLeft.MarginalReflexDistance1
        ResultsPercentile.MarginalReflexDistance2 = abs(
            ResultsLeft.MarginalReflexDistance2 - ResultsRight.MarginalReflexDistance2) * 100 / ResultsLeft.MarginalReflexDistance2
        ResultsPercentile.CommissureExcursion = abs(
            ResultsLeft.CommissureExcursion - ResultsRight.CommissureExcursion) * 100 / ResultsLeft.CommissureExcursion
        ResultsPercentile.SmileAngle = abs(
            ResultsLeft.SmileAngle - ResultsRight.SmileAngle) * 100 / ResultsLeft.SmileAngle
        if ResultsLeft.DentalShow > 0:
            ResultsPercentile.DentalShow = abs(
                ResultsLeft.DentalShow - ResultsRight.DentalShow) * 100 / ResultsLeft.DentalShow
        else:
            ResultsPercentile.DentalShow = 0

        if ResultsLeft.PalpebralFissureHeight > 0:
            ResultsPercentile.PalpebralFissureHeight = abs(
                ResultsLeft.PalpebralFissureHeight - ResultsRight.PalpebralFissureHeight) * 100 / ResultsLeft.PalpebralFissureHeight
        else:
            ResultsPercentile.PalpebralFissureHeight = 0
    else:  # right is the good side
        ResultsPercentile.BrowHeight = abs(
            ResultsLeft.BrowHeight - ResultsRight.BrowHeight) * 100 / ResultsRight.BrowHeight
        ResultsPercentile.MarginalReflexDistance1 = abs(
            ResultsLeft.MarginalReflexDistance1 - ResultsRight.MarginalReflexDistance1) * 100 / ResultsRight.MarginalReflexDistance1
        ResultsPercentile.MarginalReflexDistance2 = abs(
            ResultsLeft.MarginalReflexDistance2 - ResultsRight.MarginalReflexDistance2) * 100 / ResultsRight.MarginalReflexDistance2
        ResultsPercentile.CommissureExcursion = abs(
            ResultsLeft.CommissureExcursion - ResultsRight.CommissureExcursion) * 100 / ResultsRight.CommissureExcursion
        ResultsPercentile.SmileAngle = abs(
            ResultsLeft.SmileAngle - ResultsRight.SmileAngle) * 100 / ResultsRight.SmileAngle
        if ResultsRight.DentalShow > 0:
            ResultsPercentile.DentalShow = abs(
                ResultsLeft.DentalShow - ResultsRight.DentalShow) * 100 / ResultsRight.DentalShow
        else:
            ResultsPercentile.DentalShow = 0

        if ResultsRight.PalpebralFissureHeight > 0:
            ResultsPercentile.PalpebralFissureHeight = abs(
                ResultsLeft.PalpebralFissureHeight - ResultsRight.PalpebralFissureHeight) * 100 / ResultsRight.PalpebralFissureHeight
        else:
            ResultsPercentile.PalpebralFissureHeight = 0
