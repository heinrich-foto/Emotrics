...
def ComputeFaceLandMarks(image, ModelName):
    ...
    if ModelName == 'iBUG':  # user wants to use iBUGS model
        predictor = shape_predictor(
            os.path.join(scriptDir, 'include', 'data', 'shape_predictor_68_face_landmarks.dat'))
    elif ModelName == 'MEE':  # user wants to use MEE model
        predictor = shape_predictor(
            os.path.join(scriptDir, 'include', 'data', 'mee_shape_predictor_68_face_landmarks.dat'))
